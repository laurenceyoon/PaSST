...
def default_conf():
    ...
    models = {
        "net": DynamicIngredient("models.passt.model_ing", arch="passt_s_swa_p16_s16_128_ap473", n_classes=14, s_patchout_t=40, s_patchout_f=4),
    ...
    basedataset = DynamicIngredient("nia14.dataset.dataset", wavmix=1)
    trainer = dict(max_epochs=100, gpus=1, weights_summary='full', benchmark=True, num_sanity_val_steps=0,
...
class M(Ba3lModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_mixup:
            ...
            samples_loss = F.binary_cross_entropy_with_logits(
                y_hat, y_mix.to(y_hat.dtype), reduction="none")
            loss = samples_loss.mean()
            samples_loss = samples_loss.detach()
        else:
            samples_loss = F.binary_cross_entropy_with_logits(y_hat, y.to(y_hat.dtype), reduction="none")
            loss = samples_loss.mean()
            samples_loss = samples_loss.detach()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for net_name, net in model_name:
            y_hat, _ = net(x)
            samples_loss = F.binary_cross_entropy_with_logits(y_hat, y.to(y_hat.dtype))
    ...
    def validation_epoch_end(self, outputs):
        ...
        for net_name, net in model_name:
            ...
            _, out_idx = torch.max(out, dim=1)
            _, target_idx = torch.max(target, dim=1)
            n_correct_pred = (out_idx==target_idx).sum().detach().numpy()

            try:
                average_precision = metrics.average_precision_score(
                    target.float().numpy(), out.float().numpy(), average='weighted')
            except ValueError:
                average_precision = np.array([np.nan] * NUM_CLASSES)
                print(target)
                print(out)
                raise
            try:
                ...
                roc = np.array([np.nan] * NUM_CLASSES)
                #print(target)
                #print(out)
                #raise

            val_acc = n_correct_pred / out_idx.shape[0]
            ...
            print(f"============average_precision: {average_precision}, acc: {val_acc}================\n\n")
            self.log_dict(logs, sync_dist=True)
            ...
            # classes = [5, 6, 7, 8, 9, 10, 11, 12, 13, 55, 56, 57, 74, 75]
            classes = ["airport", "exhibitionCenter", "departmentStore", "Mart", "traditionalMarket", "undergroundShoppingArea", "parkingLot", "busStop", "busTerminal", "SF", "turnnel", "timeMachine", "naturalDisaster", "ArtificialWar"]
            # Build confusion matrix
            cf_matrix = confusion_matrix(target_idx, out_idx)
...
@ex.command
def load_and_eval(_run, _config, _log, _rnd, _seed):
    # force overriding the config, not logged = not recommended
    trainer = ex.get_trainer()
    train_loader = ex.get_train_dataloaders()
    val_loader = ex.get_val_dataloaders()
    ...
    ###### Set the path to desired checkpoint. ######
    ckptpath = './output/nia14/_2/checkpoints/epoch=149-step=38399.ckpt'
    ...
    for key in ckpt['state_dict']:
        ...
        ###### Uncomment this block if you want to use swa parameter ######
        if 'swa' in key:
          new_key = key.replace('net_swa.','')
          state_dict[new_key] = ckpt['state_dict'][key]

    modul.net.load_state_dict(state_dict)
